  ...
  async function load() {
    ...
    tableWrap.append(dataTable(el('table', {},
      ...
      el('tbody', {}, res.data.map(item => el('tr', {
        ...
        el('td', { class: 'cell-line' },
          el('div', {}, item.phone),
          item.email && el('div', { class: 'hint' }, item.email),
        ),
        el('td', { class: 'cell-excerpt', style: { maxWidth: '320px' } }, excerpt(item.message, 100)),
        el('td', { class: 'cell-status' }, el('span', { class: `pill pill-${item.status}` }, STATUS_LABELS[item.status] || item.status)),
        el('td', { class: 'hint cell-meta' }, relativeTime(item.createdAt)),
      ))),
    )))
