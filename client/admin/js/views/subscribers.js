  ...
  async function load() {
    ...
    tableWrap.append(dataTable(el('table', {},
      ...
      el('tbody', {}, res.data.map(s => el('tr', {},
        el('td', { class: 'row-title cell-primary' }, s.email),
        el('td', { class: 'cell-meta' }, s.name || '—'),
        el('td', { class: 'cell-status' }, el('span', { class: `pill pill-${s.status === 'subscribed' ? 'published' : 'draft'}` }, s.status)),
        el('td', { class: 'hint cell-meta' }, formatDate(s.createdAt)),
        el('td', { class: 'actions' },
